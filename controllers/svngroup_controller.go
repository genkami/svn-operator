...
// SVNGroupReconciler reconciles a SVNGroup object
type SVNGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// the SVNGroup object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.7.0/pkg/reconcile
func (r *SVNGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = r.Log.WithValues("svngroup", req.NamespacedName)
...
func (r *SVNGroupReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&svnv1alpha1.SVNGroup{}).
		Complete(r)
}
