...
// SVNUserReconciler reconciles a SVNUser object
type SVNUserReconciler struct {
	client.Client
	Log    logr.Logger
...
// the SVNUser object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.7.0/pkg/reconcile
func (r *SVNUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = r.Log.WithValues("svnuser", req.NamespacedName)
...
func (r *SVNUserReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&svnv1alpha1.SVNUser{}).
		Complete(r)
}
